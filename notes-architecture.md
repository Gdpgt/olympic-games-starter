...
1. La ligne "const selectedCountry = data.find((i: any) => i.country === countryName);"
...
2. Les erreurs ne sont qu'assignées à une variable, sans console.error pour les développeurs,
   ni de redirection ou d'enclenchement d'une erreur qui s'afficherait dans le html

3. Les 2 component.ts définissent la propriété olympicUrl puis font un appel http pour récupérer les
   données, alors que cela devrait être la responsabilité d'une classe dédiée OlympicService par exemple

4. De nombreuses variables ne sont pas strictement typées, elles sont de type 'any'. Le risque est
...
5. "countryName: string" devrait être "id: int" selon les spécifications
...
7. Les Observables créés via subscribe ne sont jamais fermés : lorsque le composant est détruit, ils
...
8. Des console.log sont toujours présents alors qu'ils auraient dû être enlevé après la phase de debuggage. Et les console.log présents dans les chemins d'erreur devraient être en console.error() à la place

### Erreurs relevées dans country.component.html et home.component.html et fichiers scss :

1. (quasi) aucune media-query n'existe dans les fichiers .scss, ce qui signifie que
...
2. La balise "<h2>Date</h2>" n'a rien à faire là dans country.component.html. Si l'intention était d'ajouter un texte sous l'axe des abscisses, il aurait fallu l'ajouter en option dans la fonction buildChart() du country.component.ts.

3. {{ pieChart / linechart }} n'ont rien à faire entre les balises <canvas>. Si l'intention est
